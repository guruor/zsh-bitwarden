...
- `bwfile` stores YAML metadata in secure-note notes and exact textual payloads in one hidden `content` field. Save preserves a source symlink path while reading its resolved regular-file target; load still refuses symlink paths. Keep secure same-directory atomic writes, provision-only `--all`, static completion, no payload cache/output, and no automatic source or local-file deletion.
