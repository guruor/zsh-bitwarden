...
- `bwfile` stores YAML metadata in secure-note notes. Version 2 stores exact text in ordered, bounded hidden `content_NNNN` fields of at most 3,500 UTF-8 plaintext bytes (safely below Bitwarden's post-encryption field limit), with at most 9,999 chunks; version 1 single-field `content` items remain readable. Save preserves a source symlink path while reading its resolved regular-file target; load still refuses symlink paths. Keep secure same-directory atomic writes, provision-only `--all`, static completion, no payload cache/output, and no automatic source or local-file deletion.
